...
const Booking = () => {
  ...
  const handleTimeClick = (time: string) => {
    ...
    const plusOne = `${String(parseInt(time.split(":")[0]) + 1).padStart(2, "0")}:00`;

    if (!startTime) {
      // First click: set 1-hour booking starting at this slot
      setStartTime(time);
      setEndTime(plusOne);
      return;
    }
    if (time < startTime) {
      // Clicked earlier slot: restart 1-hour selection
      setStartTime(time);
      setEndTime(plusOne);
      return;
    }
    if (time === startTime) {
      // Same start: reset to 1 hour
      setEndTime(plusOne);
      return;
    }
    if (canSelectAsEnd(time)) {
      // Extend (or shrink) end time to time+1
      setEndTime(plusOne);
    }
  };
          ...
          <div className="max-w-5xl mx-auto grid lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-5">
              ...
              <p className="text-xs text-muted-foreground mb-4">
                Klik 1 jam untuk booking 1 jam, lalu klik jam berikutnya untuk memperpanjang durasi.
              </p>
